...
pub fn export_triples(graph: &CodeGraph) -> Result<String> {
    ...
    for edge_id in 0..graph.edge_count() as u64 {
        if let Ok(edge) = graph.get_edge(edge_id) {
            output.push_str(&format!(
                "<node:{}> <edge:{}> <node:{}> .\n",
                edge.source_id, format!("{:?}", edge.edge_type), edge.target_id
            ));
...
fn format_triple_object(value: &crate::PropertyValue) -> String {
    match value {
        ...
        crate::PropertyValue::IntList(v) => {
            let joined = v.iter().map(|i| i.to_string()).collect::<Vec<_>>().join(",");
...
mod tests {
    ...
    fn test_format_triple_object() {
        ...
        assert_eq!(format_triple_object(&PropertyValue::String("hello".to_string())), "\"hello\"");
        assert_eq!(format_triple_object(&PropertyValue::Int(42)), "\"42\"^^<xsd:integer>");
        assert_eq!(format_triple_object(&PropertyValue::Bool(true)), "\"true\"^^<xsd:boolean>");
    }
