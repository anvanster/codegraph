...
use codegraph::{CodeGraph, NodeType, EdgeType, helpers, export::DotOptions};
use std::path::Path;
...
fn main() -> codegraph::Result<()> {
    ...
    let main_fn = helpers::add_function_with_metadata(
        &mut graph, main_py, "main", 1, 20, "public", "def main():", false, false
    )?;
    
    let process_fn = helpers::add_function_with_metadata(
        &mut graph, main_py, "process_user", 22, 35, "public", "def process_user(user):", false, false
    )?;
    
    let validate_fn = helpers::add_function_with_metadata(
        &mut graph, utils_py, "validate_email", 1, 10, "public", "def validate_email(email):", false, false
    )?;
    
    let format_fn = helpers::add_function_with_metadata(
        &mut graph, utils_py, "format_output", 12, 25, "private", "def format_output(data):", false, false
    )?;
    
    let test_fn = helpers::add_function_with_metadata(
        &mut graph, tests_py, "test_main", 5, 20, "public", "def test_main():", false, true
    )?;
    ...
    helpers::add_import(&mut graph, main_py, utils_py, vec!["validate_email", "format_output"])?;
    ...
    let filtered_json = graph.export_json_filtered(
    ...
    graph.export_csv(
        Path::new("output/nodes.csv"),
        Path::new("output/edges.csv")
    )?;
