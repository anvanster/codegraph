...
fn test_crash_recovery_with_wal() {
    ...
    for i in 0..99 {
        let edges = graph.get_edges_between(node_ids[i], node_ids[i + 1]).unwrap();
...
fn test_crash_recovery_with_batch_operations() {
    ...
    let (node_ids, edge_ids) = {
        ...
        let edges: Vec<_> = (0..49)
            .map(|i| {
                (node_ids[i], node_ids[i + 1], EdgeType::Contains, PropertyMap::new())
            })
            .collect();
...
fn test_partial_transaction_recovery() {
    ...
    let func_id = {
        ...
        graph.add_edge(file_id, func_id, EdgeType::Contains, PropertyMap::new()).unwrap();
