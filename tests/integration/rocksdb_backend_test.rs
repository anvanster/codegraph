...
fn test_rocksdb_persistence() {
    ...
    // Create graph and add data
    {
        ...
        node1_id = graph.add_node(NodeType::CodeFile, PropertyMap::new().with("path", "main.rs")).unwrap();
        node2_id = graph.add_node(NodeType::Function, PropertyMap::new().with("name", "main")).unwrap();
        edge_id = graph.add_edge(node1_id, node2_id, EdgeType::Contains, PropertyMap::new()).unwrap();
...
fn test_rocksdb_counter_persistence() {
    ...
    // Create graph and add nodes
    {
        ...
        graph.add_node(NodeType::Function, PropertyMap::new()).unwrap();
        last_node_id = graph.add_node(NodeType::Function, PropertyMap::new()).unwrap();
    ...
    // Reopen and add more nodes - IDs should continue from where they left off
    {
        ...
        let new_node_id = graph.add_node(NodeType::Function, PropertyMap::new()).unwrap();
...
fn test_rocksdb_delete_and_reopen() {
    ...
    // Create graph, add nodes, delete one
    {
        ...
        node1_id = graph.add_node(NodeType::Function, PropertyMap::new()).unwrap();
        node2_id = graph.add_node(NodeType::Function, PropertyMap::new()).unwrap();
        node3_id = graph.add_node(NodeType::Function, PropertyMap::new()).unwrap();
        
        graph.add_edge(node1_id, node2_id, EdgeType::Calls, PropertyMap::new()).unwrap();
        graph.add_edge(node2_id, node3_id, EdgeType::Calls, PropertyMap::new()).unwrap();
