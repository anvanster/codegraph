...
fn test_add_function() {
    ...
    let func_id = codegraph::helpers::add_function(
        &mut graph,
        file_id,
        "my_function",
        10,
...
fn test_add_class() {
    ...
    let class_id = codegraph::helpers::add_class(
        &mut graph,
        file_id,
        "Person",
        50,
...
fn test_add_method() {
    ...
    let method_id = codegraph::helpers::add_method(
        &mut graph,
        class_id,
        "get_name",
        55,
...
fn test_add_call() {
    ...
    let callee_id = codegraph::helpers::add_function(&mut graph, file_id, "helper", 12, 20).unwrap();
...
fn test_add_import() {
    ...
    let edge_id = codegraph::helpers::add_import(
        &mut graph,
        file1_id,
        file2_id,
        vec!["helper", "process"]
...
fn test_get_callees() {
    ...
    let helper1_id = codegraph::helpers::add_function(&mut graph, file_id, "helper1", 12, 20).unwrap();
    let helper2_id = codegraph::helpers::add_function(&mut graph, file_id, "helper2", 22, 30).unwrap();
