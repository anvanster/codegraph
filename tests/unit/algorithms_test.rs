...
fn test_bfs_traversal_with_max_depth() {
    ...
    let result = graph.bfs(a, codegraph::Direction::Outgoing, Some(1)).unwrap();
    assert_eq!(result.len(), 1);
    assert!(result.contains(&b));
    
    // BFS from A with depth 2 should find B and C
    let result = graph.bfs(a, codegraph::Direction::Outgoing, Some(2)).unwrap();
    assert_eq!(result.len(), 2);
    assert!(result.contains(&b));
    assert!(result.contains(&c));
    
    // BFS from A with depth 3 should find B, C, and D
    let result = graph.bfs(a, codegraph::Direction::Outgoing, Some(3)).unwrap();
...
fn test_dfs_traversal() {
    ...
    let result = graph.dfs(a, codegraph::Direction::Outgoing, Some(10)).unwrap();
    assert_eq!(result.len(), 3); // B, C, D
    
    // DFS with depth limit
    let result = graph.dfs(a, codegraph::Direction::Outgoing, Some(1)).unwrap();
...
fn test_tarjans_scc_cycle_detection() {
    ...
    let found_cycle = sccs.iter().any(|scc| {
        scc.len() == 3 && scc.contains(&a) && scc.contains(&b) && scc.contains(&c)
    });
    
    assert!(found_cycle, "Should detect the circular dependency as an SCC");
}
...
fn test_call_chain() {
    ...
    let func_a = helpers::add_function_with_metadata(
        &mut graph, file, "func_a", 1, 5, "public", "def func_a():", false, false
    ).unwrap();
    let func_b = helpers::add_function_with_metadata(
        &mut graph, file, "func_b", 7, 11, "public", "def func_b():", false, false
    ).unwrap();
    let func_c = helpers::add_function_with_metadata(
        &mut graph, file, "func_c", 13, 17, "public", "def func_c():", false, false
    ).unwrap();
...
fn test_actual_circular_imports() {
    ...
    let found = cycles.iter().any(|cycle| {
        cycle.contains(&utils) && cycle.contains(&models) && cycle.contains(&views)
    });
