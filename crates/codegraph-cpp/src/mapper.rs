...
mod tests {
    ...
    fn test_ir_to_graph_with_calls() {
        ...
        let mut ir = CodeIR::new(std::path::PathBuf::from("test.cpp"));
        ir.add_function(FunctionEntity::new("foo", 1, 5));
        ir.add_function(FunctionEntity::new("bar", 7, 10));
        ir.add_call(CallRelation::new("foo", "bar", 3));

        let mut graph = CodeGraph::in_memory().unwrap();
        let result = ir_to_graph(&ir, &mut graph, Path::new("test.cpp"));
        ...
        assert!(
            !edges.is_empty(),
            "Should have call edge between foo and bar"
        );
    ...
    fn test_property_types() {
        ...
        let mut graph = CodeGraph::in_memory().unwrap();
        let file_info = ir_to_graph(&ir, &mut graph, std::path::Path::new("test.cpp")).unwrap();
