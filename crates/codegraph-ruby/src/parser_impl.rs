//! Implementation of the CodeParser trait for Ruby
...
/// Ruby language parser implementing the CodeParser trait
...
mod tests {
    ...
    fn test_file_extensions() {
        let parser = RubyParser::new();
        assert_eq!(parser.file_extensions(), &[".rb", ".rake", ".gemspec"]);
    }

    #[test]
    fn test_can_parse() {
        let parser = RubyParser::new();
        assert!(parser.can_parse(Path::new("app.rb")));
        assert!(parser.can_parse(Path::new("Rakefile.rake")));
        assert!(parser.can_parse(Path::new("my_gem.gemspec")));
