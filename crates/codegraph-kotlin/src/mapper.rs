...
pub fn ir_to_graph(
    ...
    // Add classes (including objects, data classes, enum classes)
...
mod tests {
    ...
    fn test_ir_to_graph_empty() {
        let ir = CodeIR::new(PathBuf::from("Test.kt"));
        let mut graph = CodeGraph::in_memory().unwrap();
        let result = ir_to_graph(&ir, &mut graph, PathBuf::from("Test.kt").as_path());
    ...
    fn test_ir_to_graph_function_properties() {
        ...
        let func = FunctionEntity::new("publicFunc", 1, 5)
            ...
            .with_signature("fun publicFunc(): String");
    ...
    fn test_property_types() {
        ...
        let mut ir = CodeIR::new(PathBuf::from("test.kt"));
        ir.set_module(
            codegraph_parser_api::ModuleEntity::new("test", "test.kt", "kotlin")
                .with_line_count(100),
        );
        let func = FunctionEntity::new("test_fn", 10, 20)
            .with_signature("fun test_fn()")
            .with_visibility("public")
            .async_fn();
        ir.add_function(func);
