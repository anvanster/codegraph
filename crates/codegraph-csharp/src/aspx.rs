...
fn parse_attributes(s: &str) -> Vec<(String, String)> {
    ...
    loop {
        ...
        while chars.peek().map_or(false, |c| *c == '=' || *c == '"' || c.is_whitespace()) {
            chars.next();
        }
...
mod tests {
    ...
    fn test_extract_register_directive() {
        let source = r#"<%@ Register TagPrefix="uc" TagName="Header" Src="Controls/Header.ascx" %>"#;
        let directives = extract_directives(source);
        assert_eq!(directives.len(), 1);
